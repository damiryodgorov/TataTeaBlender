...
class DetectorView extends React.Component{
  	    ...
  	    if(lcd_type== 0){
			...
			if(vdefByMac[d.mac])
			{
    				...
    				if(this.state.interceptor){
						...
						if(vdefByMac[this.props.det.mac][0]['@defines']['INTERCEPTOR_DF']){
							pk = prodRec['Peak']
							...
							if(this.refs.dfs){
  								this.refs.dfs.setPeaks(siga,sigb)
  							}
						...
						if(this.refs.cb){
							if((this.refs.cb.state.rpeak != rpka)||(this.refs.cb.state.xpeak != xpka)||(this.refs.cb.state.phase != phaseA)||(this.refs.cb.state.rpeakb != rpkb)||(this.refs.cb.state.xpeakb != xpkb)|| (this.refs.cb.state.det_power_b != det_power_b) || (this.refs.cb.state.det_power_a != det_power_a) 
							||(this.refs.cb.state.phaseb != phaseB)||(this.refs.cb.state.phaseSpeed != phaseSpeedA)||(this.refs.cb.state.phaseSpeedB != phaseSpeedB)||(this.refs.cb.state.phaseMode != phaseWet) || (this.refs.cb.state.phaseModeB != phaseWetB)){//||(this.refs.cb.state.pled_a != pled_a)||(this.refs.cb.state.pled_b != pled_b)){
								this.refs.cb.setState({rpeak:rpka, xpeak:xpka, phase:phaseA, rpeakb:rpkb, xpeakb:xpkb, phaseb:phaseB,phaseSpeed:phaseSpeedA,phaseSpeedB:phaseSpeedB,phaseMode:phaseWet, phaseModeB:phaseWetB, det_power_a:det_power_a, det_power_b:det_power_b})
							}
	...
	sendPacket (n,v) {
		...
		if(typeof n == 'string'){
			...
			////////console.log(this.props.ip)
			var rpc = vdef[0]['@rpc_map']['KAPI_SIG_MODE_DF_WRITE']
			var pkt = rpc[1].map(function (r) {
				// body...
				if(!isNaN(r)){
					return r
	...
	//	////console.log(lg)
		...
		if(vdefByMac[this.props.det.mac][0]['@defines']['INTERCEPTOR_DF']){
			df = true;
			sensui =  <InterceptorDFSensitivityUI ref='dfs'  mac={this.props.det.mac} language={lg} sigmode={this.state.prodSettings['SigModeDF']} onSigMode={this.sendPacket} sens={this.state.prodSettings['Sens']} sensA={this.state.prodSettings['Sens_A']} sensB={this.state.prodSettings['Sens_B']} onFocus={this.onSensFocus} onRequestClose={this.onSensClose} sendPacket={this.sendPacket} refresh={this.refresh} onSens={this.onSens}/>
	...
	render () {
		...
		return (
			...
			<div className={klass} style={{overflow:'hidden', display:'block', width:956, marginLeft:'auto', marginRight:'auto', textAlign:'center', borderRadius:20,boxShadow:'0px 0px 0px 12px #818a90'}}>
				...
				<tr>
				<td style={{padding:0, height:90, overflow:'hidden',display:'inline-block'}}>
				<div  style={{display:'inline-block', width:330, height:90}}>
				...
				<td style={{padding:0, height:90, overflow:'hidden', display:'inline-block'}}>
				<div style={{display:'inline-block', width:280, height:90}}>
				...
				<div style={{display:'inline-block', width:330, height:90}}>
					<div style={{position:'relative',marginTop:5}}>
					<KeyboardInputTextButton language={this.props.language} overrideBG={true} bgColor={'rgba(200,200,200,1)'} rstyle={{backgroundColor:pled[this.props.pleds]}} label={vdefMapV2['@labels']['Signal'][this.props.language]['name']} lab2={''} onClick={this.onSig} isEditable={false} value={this.state.peak} inverted={false}/></div>
...
class IntClock extends React.Component{
	...
	render(){
		...
		return <React.Fragment><label style={{color:'#e1e1e1'}} onClick={this.toggleCK}>{dt}</label>
			...
			<Modal ref='dtsModal'>
				<DateTimeSelect language={this.props.language} setDT={this.changeDT} ref='dts'/>
			</Modal>
		</React.Fragment>
	}
		...
		*/
		...
		return (<div className='interceptorMainPageUI' style={style}>
					<table className='landingMenuTable' style={{marginBottom:-4, marginTop:-7}}>
						<tbody>
							<tr>
								<td style={{width:438}}><img style={lstyle}  src='assets/Interceptor-white-01.svg'/>{dfLab}</td><td><IntClock sendPacket={this.sendPacket} language={lg} ref='clock'/></td>
							{logintext}	{logincell}
				<td className="buttCell" style={{height:60}}><button onClick={this.gohome} className={home}/></td>
							</tr>
						</tbody>
...
class InterceptorNav extends React.Component{
	...
	render () {
		...
		var content = 				<InterceptorNavContent offline={this.props.offline} isSyncing={this.props.isSyncing} isUpdating={this.props.isUpdating} status={this.props.status} clearWarnings={this.props.clearWarnings} clearRejLatch={this.clearRejLatch} testReq={this.props.testReq} 
				...
				<SlimGraph df={false} int={true} ref='sg' canvasId={'sgcanvas2'}/>	</InterceptorNavContent>
		if(this.props.df){
	...
	render () {
		...
		return (
			...
			<div className={klass} style={{overflow:'hidden', display:'block', width:956, marginLeft:'auto', marginRight:'auto', textAlign:'center', borderRadius:20,boxShadow:'0px 0px 0px 12px #818a90'}}>
				...
				<div style={{paddingRight:10, paddingTop:5, paddingBottom:5, display:'block', width:516,marginLeft:70,paddingLeft:20}}><TickerBox ref='tbb'/>
				</div></td></tr>
				...
				<tr><td colSpan={2} style={{padding:0,display:'inline-block',overflow:'hidden', width:620}}><div style={{paddingLeft:10, paddingTop:5, paddingBottom:5, display:'block', width:516,marginLeft:-7,paddingLeft:20}}><TickerBox ref='tba'/></div></td><td style={{display:'inline-block', padding:0,width:336,overflow:'hidden'}}><div style={{width:356,height:26}}></div></td></tr>
				</tbody></table>
...
class InterceptorDFSensitivityUI extends React.Component{
	...
	selectChanged(v,i){
		this.props.onSigMode('SigModeDF',v)
	}
	onSigA(){
	...
	render(){
		var list = vdefByMac[this.props.mac][0]['@labels']['SigModeDF']['english']
		return <div>
		<div style={{textAlign:'center'}}>
				<div><KeyboardInputTextButton language={this.props.language} tooltip={vMapV2['Sens_A']['@translations'][this.props.language]['description']} label={vMapV2['Sens_A']['@translations'][this.props.language]['name']} onFocus={this.onFocus} onRequestClose={this.onRequestClose} num={true} isEditable={true} value={this.props.sens} onInput={this.onSens} inverted={false} />
				<KeyboardInputTextButton language={this.props.language} tooltip={vMapV2['SigModeDF']['@translations'][this.props.language]['description']} label={vMapV2['SigModeDF']['@translations'][this.props.language]['name']} 
				...
				<PopoutWheel ref='sgpw' vMap={this.props.vMap} language={this.props.language} index={0} interceptor={false} name={vMapV2['SigModeDF']['@translations'][this.props.language]['name']} val={[this.props.sigmode]} options={[list]} onChange={this.selectChanged}/>
			</div>
		</div>
		...
		<table style={{borderSpacing:0}}>
			<tbody><tr>
					...
					<td  style={{width:220,textAlign:'center', background:'linear-gradient(90deg, #818a90, #818a90 49.9%,#362c66 50.1%, #362c66)'}}></td>
					<td  style={{width:340, textAlign:'center', background:'#362c66'}}>
						<div style={{marginTop:15}}>
						<div>
						<KeyboardInputTextButton language={this.props.language} tooltip={vMapV2['Sens_A']['@translations'][this.props.language]['description']} label={vdefMapV2['@labels']['Channel B'][this.props.language]['name']}  onFocus={this.onFocus} onRequestClose={this.onRequestClose} num={true} isEditable={true} value={this.props.sensB} onInput={this.onSensB} inverted={true}/></div>
...
class InterceptorCalibrateUI extends React.Component{
	constructor(props) {
		super(props)
		this.state = ({rpeak:0,xpeak:0,phaseb:0,phaseSpeedB:0, phaseModeB:0, phaseSpeed:0,phase:0,phaseMode:0, tmpStr:'', tmpStrB:'', pled_a:0, pled_b:0, det_power_a:0, det_power_b:0, pa:false,pb:false})
...
class KeyboardInputTextButton extends React.Component{
	...
	render() {
		...
		var	kb = <label style={{fontSize:25, textAlign:'center', width:75, display:'inline-block', lineHeight:'57px'}} onClick={this.editValue}>{this.props.value}</label>
...
<svg style={{position:'absolute', top:14, marginLeft:-36, width:30}} onClick={this.onDelete} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path fill="#3E3E40" d="M22 3H7c-.69 0-1.23.35-1.59.88L0 12l5.41 8.11c.36.53.9.89 1.59.89h15c1.1 0 2-.9 2-2V5c0-1.1-.9-2-2-2zm-3 12.59L17.59 17 14 13.41 10.41 17 9 15.59 12.59 12 9 8.41 10.41 7 14 10.59 17.59 7 19 8.41 15.41 12 19 15.59z"/></svg>
		...
		{helpModal}
		<div hidden={hidden}><button style={{height:50, border:'5px solid #808a90', color:'#e1e1e1', background:'#5d5480', width:150, borderRadius:20}} onClick={this.onEnter}>{vdefMapV2['@labels']['Accept'][this.props.language].name}</button>
		<button style={{height:50, border:'5px solid #808a90',color:'#e1e1e1', background:'#5d5480', width:150, borderRadius:20}} onClick={this.close}>{vdefMapV2['@labels']['Cancel'][this.props.language].name}</button></div>
		</div>
...
class DateTimeSelect extends React.Component{
	...
	render(){
		...
		return <div style={{position:'relative'}}>{tpw}{dpw}
		<div>
		{titlediv}
